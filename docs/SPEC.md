...
ScanSnap 等で連続スキャンされた、複数書類が混在する PDF ファイル群を「書類グループごとに自動分割」し、「OCR テキストから日付と書類タイプを抽出して `YYYY-MM-DD_書類タイプ.pdf` 形式に自動リネーム」するツール。CUI（`psar` コマンド）と Tkinter GUI の両方を提供し、Python パッケージとして pip 配布する。
...
- 日付・書類タイプの抽出と `YYYY-MM-DD_種別.pdf` 形式への命名
- 書類タイプのプロファイルカスタマイズ
...
- **FR-3-5** 書類タイプ判定はタイトル領域（先頭6行）→ 本文の順で `DEFAULT_TITLE_PATTERNS` / `DEFAULT_BODY_PATTERNS` を適用する。OCR 誤読パターン（`MOJIBAKE_FIX`）適用後に判定する。
- **FR-3-6** デフォルトプロファイルは教会書類向け（週報・主日礼拝メッセージ要旨・会計報告 等）。ユーザはプロファイルとして正規表現リストを定義してカスタマイズできる（外部プロファイル読み込みは v0.2 以降、TOML を第一候補として検討、JSON との比較は実装時に判断）。
- **FR-3-7** 出力ファイル名は `YYYY-MM-DD_書類タイプ.pdf`（日付不明の場合は `日付不明_書類タイプ.pdf`）。同名重複時は `_01`, `_02` の連番サフィックスを付与する。
- **FR-3-8** 書類タイプが「書類」（デフォルト）のとき、ファイル名の名前部分を fallback タイトルとして使う（日付文字列と記号除去後）。
