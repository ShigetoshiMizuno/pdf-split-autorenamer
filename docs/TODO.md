...
- [ ] `pdfio.extract_text_pymupdf` および `save_pdf_pages` で `fitz.open(pdf_path)` を直接呼び出している箇所を、`fitz.open(stream=path.read_bytes(), filetype="pdf")` 経由に変更
- [ ] `analyze.py:collect_pages` の `fitz.open(pdf_path)` も同様に変更
- [ ] `pyproject.toml` の PyMuPDF バージョンを `>=1.23,<1.26` に固定し、互換性問題に備える
- [ ] CI（GitHub Actions windows-latest）で日本語パスを含むテストフィクスチャの動作確認
- [ ] 移行後、KNOWLEDGE.md §2 を更新

### T-07b: phash 関連コード削除
- [Issue #7] [担当: PRGちゃん] [推定: 0.25日] [依存: なし]
- [ ] `pdfio.avg_phash`, `pdfio.hamming` を削除
- [ ] `analyze.py:collect_pages` の `ph = avg_phash(page)` および `pages` の `phash` フィールドを削除
- [ ] `score_boundary` の reasons から phash 言及を削除
- [ ] HTML レポートのテンプレートには影響しない（参照していない）
- [ ] SPEC.md・KNOWLEDGE.md の phash 記述を「将来検討」から「削除済み（重複検出機能として再実装する場合は別 Issue）」に更新

### T-01: Tesseract フォールバック実装
- [Issue #3] [担当: PRGちゃん] [推定: 2〜3日]
- [ ] `pdfio.has_text_layer(pdf_path: Path) -> bool` を追加
  - 各ページの `page.get_text().strip()` が空かどうかで判定
  - 全ページ空 → False、1ページでも有テキスト → True
- [ ] `extract_text_tesseract(image: bytes, lang: str = "jpn") -> str` を `pdfio.py` に追加
  - `tesseract stdin stdout -l jpn --psm 3` をサブプロセス呼び出し
- [ ] `find_tesseract() -> str | None` を追加（PATH + Windows 既知パス）
- [ ] `extract_text()` に `ocr_fallback: bool = True` パラメータを追加
  - テキストレイヤーなしページのみ Tesseract を呼ぶ
- [ ] CLI `psar analyze / rename` に `--no-ocr-fallback` オプション追加
- [ ] `jpn` traineddata 不在時のエラーメッセージ（インストール URL 付き）
- [ ] README に「OCR フォールバック」セクション追加

### T-02: 外部プロファイル読み込み（TOML）
- [Issue なし（SPEC FR-3-6）] [担当: PRGちゃん] [推定: 1日]
- [ ] プロファイル TOML フォーマットの定義（title_patterns / body_patterns の正規表現リスト）
- [ ] `textops.load_profile(path: Path) -> tuple[list, list]` を追加
- [ ] `psar rename --profile <path>` オプション追加
- [ ] GUI にプロファイル選択欄追加
- [ ] サンプルプロファイル（`profiles/church.toml`、`profiles/business.toml`）を同梱

### T-03: GitHub Actions 自動ビルド ※ v0.2.5（v0.2 完了後に着手）
- [Issue #2] [担当: PRGちゃん] [推定: 3〜5日] [依存: T-10a]
- [ ] `.github/workflows/build-release.yml` 作成
  - トリガー: `push tags: ['v*']`
  - matrix: `windows-latest` / `macos-latest` / `ubuntu-latest`
- [ ] PyInstaller ビルドコマンドの確定（既に `__main__.py` あり）
- [ ] Poppler 同梱方針決定（案 A: 同梱なしが推奨）
- [ ] `softprops/action-gh-release` で Releases に自動アップロード
- [ ] README に「ダウンロード」セクション追加
- [ ] **検討事項**（v1.0 持ち越し可）:
...
- [ ] `MOJIBAKE_FIX` を外部 TOML（`profiles/scansnap-s500.toml` 等）に切り出す
- [ ] `textops.load_mojibake_map(path)` を追加し、プロファイル経由で差し替え可能に
- [ ] `extract_kind` 内の OCR 化け候補列挙（`歓迎|歎迎|藪迎|裁迎|...`）を、`fix_mojibake` 適用後に正字パターン（`歓迎` のみ）でマッチする方針に統一
- [ ] 実装メモ: 現状 5 エントリのみ。ScanSnap S500 特有パターン。機種依存のため外部化は v0.2 必須

### T-04b: `_KEEP_CHARS` 未使用問題の解消
- [Issue なし] [担当: PRGちゃん] [推定: 0.25日] [依存: なし]
- [ ] `textops.py` の `_KEEP_CHARS` 正規表現が定義のみで使われていない
- [ ] 方針決定: 削除推奨（既存の `_INVALID_NAME` ベースのサニタイズで十分実用に足りているため）
- [ ] 削除する場合、KNOWLEDGE.md §6 の「`_KEEP_CHARS` 未使用問題」セクションを「解消済み」に更新

### T-10a: テスト整備（v0.2 並走分）
- [Issue なし] [担当: QAちゃん/PRGちゃん] [推定: 1〜2日] [依存: T-00]
- [ ] `tests/test_textops.py`: `fix_mojibake` / `fix_broken_unicode` / `extract_dates_all` / `extract_kind` / `sanitize_filename`
- [ ] `tests/test_pdfio.py`: 最小サンプル PDF（自前生成）で `extract_text_pymupdf` / `save_pdf_pages` を検証
- [ ] GitHub Actions に pytest ジョブ追加（windows / ubuntu）
- [ ] 目標: コアモジュールカバレッジ 60%（T-10 で v1.0 までに 70% へ引き上げ）

### T-10b: logging 移行
- [Issue なし] [担当: PRGちゃん] [推定: 0.5日] [依存: なし]
- [ ] `cli.py` / `analyze.py` / `split.py` / `rename.py` / `gui.py` の `print` を `logging` に置換
- [ ] `--verbose` (DEBUG) / `--quiet` (WARNING) オプション追加
- [ ] GUI のログエリアに `logging.Handler` を接続（CUI と同一フォーマット）
- [ ] エラー分類（致命/警告/情報）を SPEC FR-7-3 に従って整理
