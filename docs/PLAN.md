...
- [ ] Issue #6（PyMuPDF 入力側 ANSI）が修正され、CI で日本語パス + Windows のテストが通る
- [ ] phash 関連コードが削除されているか、重複検出機能として実装されている
- [ ] `has_text_layer()` が OCR なし PDF を正しく判定できる（テストケース: 全ページ画像、混在、全ページテキスト の3種）
- [ ] Tesseract で日本語 PDF のテキスト抽出が機能する
- [ ] `--no-ocr-fallback` で Tesseract を無効化できる
- [ ] 外部プロファイル（TOML）を指定してカスタム書類タイプ判定ができる
- [ ] `--verbose` / `--quiet` でログレベルが切り替わる
- [ ] コアモジュールのテストカバレッジ 60%
...
- [ ] タグ push 時に 3 プラットフォームの実行ファイルが Releases に公開される
- [ ] 各プラットフォームの実行ファイルがダブルクリック起動で GUI を表示する
